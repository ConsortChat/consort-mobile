...
data class NotificationChannel (
...
)
 {
  ...
  override fun toString(): String {
    return "NotificationChannel(id=$id, importance=$importance, name=$name, lightsEnabled=$lightsEnabled, soundUrl=$soundUrl, vibrationPattern=${vibrationPattern?.contentToString()})"
  }
}
...
data class Person (
...
)
 {
  ...
  override fun toString(): String {
    return "Person(iconBitmap=${iconBitmap?.contentToString()}, key=$key, name=$name)"
  }
}
...
interface AndroidNotificationHostApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: AndroidNotificationHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.zulip.AndroidNotificationHostApi.requestNotificationPermission$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.requestNotificationPermission{ result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(AndroidNotificationsPigeonUtils.wrapError(error))
              } else {
                val data = result.getOrNull()
                reply.reply(AndroidNotificationsPigeonUtils.wrapResult(data))
              }
            }
          }
        } else {

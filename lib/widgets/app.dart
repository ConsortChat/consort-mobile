...
class _ZulipAppState extends State<ZulipApp> with WidgetsBindingObserver {
  static const _windowsDeepLinkChannel =
      MethodChannel('org.zulip.flutter/deep_links');
  ...
  void initState() {
    ...
    if (defaultTargetPlatform == TargetPlatform.windows) {
      _windowsDeepLinkChannel.setMethodCallHandler(_handleWindowsDeepLink);
    }
  ...
  void dispose() {
    if (defaultTargetPlatform == TargetPlatform.windows) {
      _windowsDeepLinkChannel.setMethodCallHandler(null);
    }
    WidgetsBinding.instance.removeObserver(this);
    super.dispose();
  }

  Future<void> _handleWindowsDeepLink(MethodCall call) async {
    if (call.method != 'handleWebAuthUrl' || call.arguments is! String) {
      return;
    }
  ...
  Future<bool> _handleIncomingUrl(Uri url) async {
    switch (url) {
      case Uri(scheme: 'zulip', host: 'login'):
  ...
  Future<bool> didPushRouteInformation(routeInformation) async {
    if (await _handleIncomingUrl(routeInformation.uri)) return true;

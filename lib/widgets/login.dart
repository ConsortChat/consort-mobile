...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> handleWebAuthUrl(Uri url) async {
    var stage = 'browser cleanup';
    setState(() {
      _inProgress = true;
    });
    try {
      if (await ZulipBinding.instance.supportsCloseForLaunchMode(
          LaunchMode.inAppBrowserView)) {
        await ZulipBinding.instance.closeInAppWebView();
      }

      stage = 'active login state';
      ...
      if (payload.realm.origin != widget.serverSettings.realmUrl.origin) {
        // A reverse proxy can give the realm a different internal origin from
        // the public origin used to start login.  Accept that case only after
        // proving the returned credentials work at the public origin and refer
        // to the same user as the callback.
        stage = 'realm credential validation';
        final userId = await _getUserId(payload.email, apiKey);
        if (userId != payload.userId) throw Error();
      }
      ...
      if (e is PlatformException && e.message != null) {
        ...
        message = '$message\n\nDiagnostic stage: $stage (${e.runtimeType}).';
      }
      showErrorDialog(context: context,

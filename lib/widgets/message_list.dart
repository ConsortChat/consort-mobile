  ...
  static AppBar build(BuildContext context, {required Narrow narrow}) {
    ...
    switch (narrow) {
      ...
      case DmNarrow(:final allRecipientIds):
        if (store.consort.enabled) {
          actions.add(ConsortCallButton(
            channelId: null,
            userIds: allRecipientIds,
            loungeRoomId: null,
            subject: 'Direct message call', // skip-i18n
            compact: true));
        }
      case ChannelNarrow(:final channelId):
        final channel = store.subscriptions[channelId];
        if (store.consort.enabled && channel?.voiceVideoEnabled == true) {
          actions.add(ConsortCallButton(
            channelId: channelId,
            userIds: null,
            loungeRoomId: null,

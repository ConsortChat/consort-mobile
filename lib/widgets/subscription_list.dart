...
class _SubscriptionListPageBodyState extends State<SubscriptionListPageBody> with PerAccountStoreAwareStateMixin<SubscriptionListPageBody> {
  ...
  void onNewStore() {
    ...
    callsModel?.removeListener(_modelChanged);
    callsModel?.stopPolling();
    callsModel = PerAccountStoreWidget.of(context).consort
      ..addListener(_modelChanged)
      ..startPolling();
  }
  ...
  void _handleChannelSelect(ChannelNarrow narrow) {
    if (widget.onChannelSelect case final onChannelSelect?) {
      ...
      final channel = PerAccountStoreWidget.of(context)
        .subscriptions[narrow.channelId];
      final route = channel?.isLounge == true
        ? LoungePage.buildRoute(context: context, channelId: narrow.channelId)
        : channel?.voiceVideoEnabled == true && channel?.textChatDisabled == true
          ? VoiceChannelPage.buildRoute(context: context, channelId: narrow.channelId)
...
class SubscriptionItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final occupancy = PerAccountStoreWidget.of(context)
      .consort.occupancyByChannel[subscription.streamId];
    return Material(
      // TODO(design) check if this is the right variable
      color: designVariables.background,
      child: InkWell(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (occupancy != null) Padding(
            padding: const EdgeInsets.fromLTRB(39, 0, 16, 8),
            child: ConsortOccupancyStrip(occupancy: occupancy)),
        ])));
  }
}

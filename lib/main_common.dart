...
/// The `main` function shared by each build of the app:
/// lib/main.dart for Google Play and iOS, and lib/main_fdroid.dart for F-Droid.
void runMain(List<String> args, {
  required RemotePushNotifications? remotePushNotifications,
}) {
  if (args.contains(WebPushService.kBackgroundEngineArg)) {
    _mainInBackground();
    return;
  }
  mainInit(remotePushNotifications: remotePushNotifications);
  runApp(const ZulipApp());
}

/// What [runMain] does in the headless engine the UnifiedPush plugin starts
...
/// Everything [runMain] does short of [runApp].
void mainInit({required RemotePushNotifications? remotePushNotifications}) {
  assert(() {
    debugLogEnabled = true;
    return true;
  }());

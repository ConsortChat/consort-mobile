...
constexpr char kDeepLinkChannel[] = "org.zulip.flutter/deep_links";
...
FlutterWindow::MessageHandler(HWND hwnd, UINT const message,
  ...
  switch (message) {
    case WM_COPYDATA: {
      ...
      if (copy_data == nullptr || copy_data->dwData != kWebAuthMessageId ||
          copy_data->lpData == nullptr || copy_data->cbData < 2) {
        break;
      }

      const auto* data = static_cast<const char*>(copy_data->lpData);
      const size_t data_length = copy_data->cbData;
      if (data[data_length - 1] != '\0') {
        break;
      }

      const std::string url(data, data_length - 1);
